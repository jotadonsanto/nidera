  ...
  (
    {
      children,
      className,
      separator = <FontAwesomeIcon icon={faArrowRight} size="xs" className="text-primary-foreground h-4 w-4 rtl:rotate-180" />,
      addSeparator = true,
      ...props
    },
    forwardedRef
...
>(({ className, as: asComp, isCurrentPage, ...props }, forwardedRef) => {
  ...
  return (
    <Comp
      className={cn(
        "text-sm underline underline-offset-4 [&:not([aria-current])]:font-light aria-[current]:font-bold [&:not([aria-current])]:hover:underline",
        className
      )}
...
>(({ className, ...props }, forwardedRef) => {
  return (
    <span
      className={cn("mx-2 opacity-100", className)}
      role="presentation"
      {...props}
      ref={forwardedRef}
    />
